package com.myflexhero.network.core.util
{
	public class DashedLine
	{
		...
		public function lineTo(x:Number,y:Number):void {
			...
			var segLength:Number = this.lineLength(this.pen.x, this.pen.y, x, y);
			if (this.overflow){
		...
		private function curveLength(sx:Number, sy:Number, cx:Number, cy:Number, ex:Number, ey:Number, accuracy:Number=-1):Number {
			var total:Number = 0;
			var tx:Number = sx;
			var ty:Number = sy;
			var px:Number, py:Number, t:Number, it:Number, a:Number, b:Number, c:Number;
			var n:Number = (accuracy > 0) ? accuracy : this._curveaccuracy;
